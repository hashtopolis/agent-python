...
class Initialize:
    ...
    def __login(self):
        ...
        if ans is None:
            ...
            if (
                "multicastEnabled" in ans
                and ans["multicastEnabled"]
                and self.get_os() == 0
    ...
    def __update_information(self):
        ...
        if Initialize.get_os() == 0:  # linux
            ...
            for line in output:
                line = line.strip()
                if not line.startswith("model name") and not line.startswith(
                    "physical id"
                ):
                    continue
            ...
            if not self.config.get_value("cpu-only"):
                try:
                    output = subprocess.check_output(
                        "lspci | grep -E 'VGA compatible controller|3D controller'",
                        shell=True,
                    )
    ...
    def __check_mtls(self, args):
        if self.config.get_value("use_mtls"):
            logging.debug("Using mTLS..")
            client_cert = self.config.get_value("mtls_cert")
            client_key = self.config.get_value("mtls_key")
            ca_cert = self.config.get_value("mtls_ca_cert")
            if args.mtls_cert is not None:
                client_cert = os.path.abspath(args.mtls_cert)
                logging.debug("Setting mtls_cert to: " + client_cert)
                self.config.set_value("mtls_cert", client_cert)
            if args.mtls_key is not None:
                client_key = os.path.abspath(args.mtls_key)
                logging.debug("Setting mtls_key to: " + client_key)
                self.config.set_value("mtls_key", client_key)
            if args.mtls_ca_cert is not None:
                ca_cert = os.path.abspath(args.mtls_ca_cert)
                logging.debug("Setting mtls_ca_cert to: " + ca_cert)
                self.config.set_value("mtls_ca_cert", ca_cert)
            if client_cert is not None and client_key is not None and ca_cert is not None:
                Session().s.cert = (client_cert, client_key)
                Session().s.verify = ca_cert
                Session().using_mtls = True
